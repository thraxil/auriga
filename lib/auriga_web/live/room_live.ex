...
defmodule AurigaWeb.RoomLive do
  ...
  defp m_to_message(%Message{type: "user", content_html: content_html, inserted_at: inserted_at, user: user}) do
  ...
  def handle_event("submit_message", %{"chat" => %{"message" => message}}, socket) do
    ...
    message = %{content: content_html, email: socket.assigns.current_user.email, inserted_at: Timex.now()}
  ...
  def handle_info(%{event: "presence_diff", payload: %{joins: joins, leaves: leaves}}, socket) do
    join_messages = joins
    |> Map.keys()
    |> Enum.map(fn email -> %{type: :system, content: "<p>#{email} joined</p>", inserted_at: Timex.now()} end)

    _join_inserts = joins
    |> Map.keys()
    |> Enum.map(fn email -> Repo.insert!(%Message{type: "system", room: socket.assigns.room,
                                                 content_md: "#{email} joined",
                                                 content_html: "<p>#{email} joined</p>"}) end)

    leave_messages = leaves
    |> Map.keys()
    |> Enum.map(fn email -> %{type: :system, content: "<p>#{email} left</p>", inserted_at: Timex.now()} end)

    _leave_inserts = leaves
    |> Map.keys()
    |> Enum.map(fn email -> Repo.insert!(%Message{type: "system", room: socket.assigns.room,
                                                 content_md: "#{email} left",
                                                 content_html: "<p>#{email} left</p>"}) end)
