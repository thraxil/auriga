...
defmodule AurigaWeb.RoomLive do
  ...
  def display_message(%{type: :system, content: content, inserted_at: inserted_at} = assigns) do
    ~H"""
    <div class="message system-message"><i><%= Timex.format!(inserted_at, "{YYYY}-{0M}-{0D} {h24}:{m}") %></i> <%= raw content %></div>
    """
  end

  def display_message(%{email: email, content: content, inserted_at: inserted_at} = assigns) do
    ~H"""
